...
class PositionalEncoding(nn.Module):
    def __init__(self, hidden_size, max_len=300):
        """
...
class EncoderBlock(nn.Module):
    def __init__(self, device, hidden_size: int, len_sentence: int, num_convs=4, kernel_size=7, p_dropout=0.1,
                 num_heads=8):
    ...
    def forward(self, x, mask):
        ...
        attention_out = self.dropout(
            self.self_attention(norm_out, norm_out, norm_out, need_weights=False, key_padding_mask=mask)[
                0])  # mask? See AllenNLP
        attention_scores = self.dropout(
            self.self_attention(norm_out, norm_out, norm_out, need_weights=True, key_padding_mask=mask)[1])
        sublayer_count += 1
        output = self.residual_with_layer_dropout(
