...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        # print(f"THIS IS BANGLIU MASK {mask} and its query {query}")
...
class SelfAttention(nn.Module):
    ...
    def dot_product_attention(self, q, k ,v, bias = False, mask = None):
        ...
        # print(f"Unmasked_logits {logits}")
        if bias:
            logits += self.bias
        if mask is not None:
            ...
            # print(f"Masked_logits {logits}")  Should be fine ...
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x, mask, l=1, blks=1):
...
class CQAttention(nn.Module):
    ...
    def forward(self, C, Q, Cmask, Qmask):
        ...
        S = self.trilinear_for_attention(C, Q) # S_ij: how much similar are Context_i and Question_j
        Cmask = Cmask.view(batch_size_c, Lc, 1)
        Qmask = Qmask.view(batch_size_c, 1, Lq)
        S1 = F.softmax(mask_logits(S, Qmask), dim=2)
        S2 = F.softmax(mask_logits(S, Cmask), dim=1) # If last row is masked, I expect the row Equal to 0 ==> S_-1 not similar to anything since it is simple pad char
        # print(f"S2: {S2}") # last row of zeros
        A = torch.bmm(S1, Q)
...
class QANet(nn.Module):
    ...
    def forward(self, Cwid, Ccid, Qwid, Qcid):
        ...
        Ce = self.emb_enc(C, maskC, 1, 1) # N, E, S = source sentence length
        Qe = self.emb_enc(Q, maskQ, 1, 1) # N, E, Q = question sentence length
