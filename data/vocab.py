...
# TODO define embedding directories
def load_vocabs(config):
    LOG.debug("Loading vectors")
    common = Vocab("common", config.get_common_embeddings())
    LOG.debug("Common vectors: %s", str(common.vectors.size()))

    vocabs = dict()
    for lang in config.languages:
        vocabs[lang] = Vocab(lang, config.get_embeddings(lang), base=common)
...
class Vocab:
    ...
    def __init__(self, language, emb_path, emb_dim, base=None):
        """
        Creates a new vocab instance.
        :param language:    vocab language in BCP-47
    ...
    def _load_vectors(self, path):
        ...
        with io.open(path, mode="r", encoding="utf-8", newline="\n", errors="strict") as f:
            ...
            n, d = map(int, f.readline().split()) # read header
